...
def load_workers_monthly_attendance_data(
        ...
        worker_details_json_path: str) -> List[Dict[str, Any]]:
    """Load workers data from Excel and JSON files."""
    try:
        return from_xls_to_json(monthly_attendance_report_xsl_path,
...
def write_worker_data(ws: openpyxl.worksheet.worksheet.Worksheet, worker: Dict[str, Any], start_row: int) -> int:
    ...
    ws.cell(row=start_row + 2, column=2, value=worker['hours']).font = TEXT_FONT
    ws.cell(row=start_row + 2, column=2).border = THICK_BORDER
    ws.cell(row=start_row + 2, column=3, value=worker['per_hour']).font = TEXT_FONT
    ws.cell(row=start_row + 2, column=3).border = THICK_BORDER
    ws.cell(row=start_row + 2, column=4, value=worker['reg_hours_sal']).font = TEXT_FONT
    ...
    ws.cell(row=start_row + 4, column=2, value=worker['hours_125']).font = TEXT_FONT
    ws.cell(row=start_row + 4, column=2).border = THICK_BORDER
    ws.cell(row=start_row + 4, column=3, value=worker['per_hour_125']).font = TEXT_FONT
    ws.cell(row=start_row + 4, column=3).border = THICK_BORDER
    ws.cell(row=start_row + 4, column=4, value=worker['extra_hours_sal']).font = TEXT_FONT
    ...
    ws.cell(row=start_row + 6, column=4, value=worker['trans_expanses']).font = TEXT_FONT
    ...
    ws.cell(row=start_row + 7, column=2, value=worker['total_hours']).font = TEXT_FONT
    ws.cell(row=start_row + 7, column=2).border = THICK_BORDER
    ws.cell(row=start_row + 7, column=3).border = THICK_BORDER
    ws.cell(row=start_row + 7, column=4, value=worker['total_sal']).font = TEXT_FONT
    ...
    ws.cell(row=start_row + 9, column=2, value=worker['work_days']).font = TEXT_FONT
    ...
    ws.cell(row=start_row + 10, column=2, value=worker['holidays']).font = TEXT_FONT
    ...
    ws.cell(row=start_row + 11, column=2, value=worker['holiday_present']).font = TEXT_FONT
    ...
    ws.cell(row=start_row + 12, column=2, value=worker['sick_days']).font = TEXT_FONT
    ...
    ws.cell(row=start_row + 13, column=2, value=worker['vac_days']).font = TEXT_FONT
    ws.cell(row=start_row + 13, column=2).border = THICK_BORDER

    # Write absense hours row in the 14th row of the table
    ws.cell(row=start_row + 14, column=1, value='שעות להוריד').font = HEADER_FONT
    ws.cell(row=start_row + 14, column=1).border = THICK_BORDER
    ws.cell(row=start_row + 14, column=2, value=worker['absense_hours']).font = TEXT_FONT
...
def set_column_widths(ws: openpyxl.worksheet.worksheet.Worksheet) -> None:
...
def main():
    workers = load_workers_monthly_attendance_data(MONTHLY_ATTENDANCE_REPORT_XLS_PATH, WORKER_DETAILS_JSON_PATH)
    wb = create_workbook()
    ws = wb.active
